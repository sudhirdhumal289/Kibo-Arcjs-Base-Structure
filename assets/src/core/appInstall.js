...
function registerCustomRoutes(actionIds, entrypointManifest, context) {
  ...
  return tenant.sites.map((site) => new Promise((resolve, reject) => {
    ...
    customRoutesApi.getCustomRouteSettings().then((currentRoutesConfig) => {
      ...
      customRoutesApi.updateCustomRouteSettings(customRouteConfig).then(resolve).catch(reject);
    }).catch(reject);
  }));
}
...
function postInstallAction(context, actions, currentIndex) {
  return new Promise((resolve) => {
    ...
    function nextTick() {
      postInstallAction(context, actions, currentIndex + 1).then(resolve).catch((error) => {
        logger.error({
...
function postInstall(context, callback) {
  ...
  postInstallAction(context, PostInstall.postInstallActions, 0).then(() => {
...
module.exports = (context, callback) => {
  ...
  const entrypointManifest = require('../bundles/entrypoint.manifest');
  const actionIds = Object.keys(entrypointManifest);

  function postRegisterActions() {
    Promise.all(registerCustomRoutes(actionIds, entrypointManifest, context)).then(() => {
      postInstall(context, callback);
    }).catch(() => {
      postInstall(context, callback);
    });
  }

  registerActions(actionIds, entrypointManifest, context).then(postRegisterActions).catch(postRegisterActions);
};
